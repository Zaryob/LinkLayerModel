...
impl OutputVariables {
    fn new(num_nodes: usize) -> OutputVariables {
        OutputVariables {
            node_pos_x: Vec::with_capacity(num_nodes),
            node_pos_y: Vec::with_capacity(num_nodes),
            output_power: Vec::with_capacity(num_nodes),
            noise_floor: Vec::with_capacity(num_nodes),
            gen: vec![Vec::with_capacity(num_nodes); num_nodes],
            pr: vec![Vec::with_capacity(num_nodes); num_nodes],
        }
    }
}
...
fn obtain_topology(in_var: &InputVariables, out_var: &mut OutputVariables) -> bool {
    ...
    match in_var.top {
        ...
        2 => {
            ...
            for i in 0..in_var.num_nodes {
                out_var.node_pos_x[i as usize] = (i % nodes_x) as f64 * cell_length + normal.sample(&mut rand::thread_rng()) * cell_length;
                out_var.node_pos_y[i as usize] = (i / nodes_x) as f64 * cell_length + normal.sample(&mut rand::thread_rng()) * cell_length;
        ...
        3=>{
            ...
            for i in 0..in_var.num_nodes {
                let mut wrong_placement = true;
                while wrong_placement {
                    out_var.node_pos_x[i as usize] =  normal.sample(&mut rand::thread_rng()) * in_var.xterr;
                    out_var.node_pos_y[i as usize] =  normal.sample(&mut rand::thread_rng()) * in_var.yterr;
                    wrong_placement = false;
                    for j in 0..i {
                        let x_dist = out_var.node_pos_x[i as usize] - out_var.node_pos_x[j as usize];
                        let y_dist = out_var.node_pos_y[i as usize] - out_var.node_pos_y[j as usize];
        ...
        4 => {
            // create topology
            // readTopologyFile(in_var.topFile, in_var, out_var);
...
fn correct_topology(in_var: &InputVariables, out_var: &OutputVariables) -> bool {
    for i in 0..in_var.num_nodes {
        for j in (i + 1)..in_var.num_nodes {
            ...
            let dist = (x_dist * x_dist + y_dist * y_dist).sqrt();
            if dist < in_var.d0 {
                println!("Error: file {} contains inter_node distances less than one.", in_var.top_file);
                std::process::exit(1);
            }
        }
    }
...
fn obtain_rssi(in_var: &InputVariables, out_var: &mut OutputVariables) -> bool {
    ...
    for i in 0..in_var.num_nodes {
        for j in i + 1..in_var.num_nodes {
            ...
            let avg_decay = -in_var.pld0
                - 10.0 * in_var.n * (dist / in_var.d0).log2() / std::f64::consts::LOG10_2 + normal.sample(&mut rand::thread_rng()) * in_var.sigma;
    ...
    for i in 0..in_var.num_nodes {
        for j in 0..in_var.num_nodes {
            let dec = in_var.pld0 + 10.0 * in_var.n * out_var.node_pos_x[j as usize].log2() / std::f64::consts::LOG10_2 + normal.sample(&mut rand::thread_rng()) * in_var.sigma;
    ...
    for i in 0..in_var.num_nodes {
        for j in 0..in_var.num_nodes {
            if i == j {
                ...
                let pre_seq = (1.0 - out_var.gen[i as usize][j as usize]).powf(8.0 * in_var.pre);
                match in_var.enc {
                    1 => {
                        // NRZ
                        out_var.gen[i as usize][j as usize] = pre_seq * (1.0 - out_var.gen[i as usize][j as usize]).powf(8.0 * (in_var.fra - in_var.pre));
                    }
                    2 => {
                        // 4B5B
                        out_var.gen[i as usize][j as usize] = pre_seq * (1.0 - out_var.gen[i as usize][j as usize]).powf(8.0 * (in_var.fra - in_var.pre) * 1.25);
                    }
                    3 => {
                        // Manchester
                        out_var.gen[i as usize][j as usize] = pre_seq * (1.0 - out_var.gen[i as usize][j as usize]).powf(8.0 * (in_var.fra - in_var.pre) * 2.0);
                    }
                    4 => {
                        // SECDED
                        out_var.gen[i as usize][j as usize] = pre_seq * ((1.0 - out_var.gen[i as usize][j as usize]).powi(8) + 8.0 * out_var.gen[i as usize][j as usize] * (1.0 - out_var.gen[i as usize][j as usize]).powi(7)).powf((in_var.fra - in_var.pre) * 3.0);
                    }
                    _ => {
    ...
    for i in 0..in_var.num_nodes {
        for j in 0..in_var.num_nodes {
            if i == j {
                ...
                let snr = (10.0f64.powf((out_var.gen[i as usize][j as usize] - out_var.noise_floor[j as usize]) / 10.0)) / 0.64;
                ...
                match in_var.mod_ {
                    1 => {
                        // NCASK
                        out_var.gen[i as usize][j as usize] = 0.5 * (snr.exp() * -0.5 + Q(snr.sqrt()));
                    }
                    2 => {
                        // ASK
...
fn Q(z: f64) -> f64 {
    ...
    if z >= 0.0 {
        (a1 * t + a2 * t.powi(2) + a3 * t.powi(3) + a4 * t.powi(4) + a5 * t.powi(5)) * (-z.powi(2) / 2.0).exp()
    } else {
